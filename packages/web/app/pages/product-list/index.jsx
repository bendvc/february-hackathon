...
ProductList.getProps = async ({res, params, location, api}) => {
    ...
    // BUG: First value is sometimes undefined
    if (searchParams.refine[0] === undefined) {
        searchParams.refine = searchParams.refine.slice(1)
    }
    ...
    const GET_CATEGORY_AND_PRODUCT = gql`
      query {
        getCategory(options: {parameters: {id: "${categoryId}"}}) {
          id
          name
          categories {
            id,
    ...
    const {getCategory, productSearch} = data

    return {searchQuery: q, productSearchResult: productSearch}
}
